...
class DBService {
    ...
    getSearchResult(query) {
        this.url = `${SERVER}/search/tv/?api_key=${API_KEY}&language=he-HE&query=${query}`
    ...
    getTvShow(id) {
        this.url = `${SERVER}/tv/${id}?api_key=${API_KEY}&language=he-HE`
        return this.getData(this.url)
    }

    getTopRated() {
        this.url = `${SERVER}/tv/top_rated?api_key=${API_KEY}&language=he-HE`
        tvShow.append(loading)
        return this.getData(this.url)
    }

    getPopular() {
        this.url = `${SERVER}/tv/popular?api_key=${API_KEY}&language=he-HE`
        tvShow.append(loading)
        return this.getData(this.url)
    }
    getToday() {
        this.url = `${SERVER}/tv/airing_today?api_key=${API_KEY}&language=he-HE`
        tvShow.append(loading)
        return this.getData(this.url)
    }
    getWeek() {
        this.url = `${SERVER}/tv/on_the_air?api_key=${API_KEY}&language=he-HE`
        tvShow.append(loading)
        return this.getData(this.url)
    }
...
leftMenu.addEventListener('click', event => {
    ...
    if (target.closest('#search')) {
        ...
        tvShowsHead.textContent = 'הזן מילת חיפוש'
    }
        ...
        dbservice.getTvShow(id)
            .then(({ poster_path, name, genres, vote_average, overview, homepage }) => {
                ...
                if (homepage) {
                    ...
                    modalLink.textContent = 'אין עמוד רשמי'
                }
            }).then(() => {
...
const renderCards = (response, target) => {
    ...
    showsList.innerHTML = ''
    tvShowsHead.textContent = target ? target.textContent : `תוצאות `
    if (results.length) {
        results.forEach(({
            vote_average: vote,
            poster_path: poster,
        ...
        loading.remove()
        tvShowsHead.textContent = 'לא נימצא כלום'
        tvShowsHead.style.cssText = 'color: red'
        // showsList.innerHTML = '<span>לא נימצא כלום</span>'
    }
}
